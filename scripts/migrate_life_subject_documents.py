#!/usr/bin/env python3
"""Copy declared subject documents to MySQL and prove a reverse export."""
...
def _arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Copy exact-byte declared subject documents from an immutable "
            "candidate snapshot into a fenced MySQL shadow and optionally "
            "reverse-export them to a new directory."
        )
    )
    parser.add_argument("--snapshot", type=Path, required=True)
    parser.add_argument("--run-id", required=True)
    parser.add_argument("--owner-id", default="subject-document-migrator")
    parser.add_argument("--reverse-export", type=Path)
    parser.add_argument("--lease-seconds", type=int, default=7200)
    parser.add_argument(
        "--schema-only",
        action="store_true",
        help="apply fenced subject schema migrations without copying documents",
    )
    ...
    try:
        run = await registry.create_run(
            ...
            metadata={
                ...
                "selection_contract": "declared-subject-paths-v1",
        ...
        copy_report = await copy_subject_documents_from_snapshot(
            args.snapshot,
            store,
        ...
        if args.reverse_export is not None:
            reverse_report = await export_subject_documents(
                store,
                args.reverse_export,
            )
        verification = {
...
def main() -> None:
    args = _arguments()
    print(canonical_json(asyncio.run(_run(args))))
