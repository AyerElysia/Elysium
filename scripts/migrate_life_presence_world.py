#!/usr/bin/env python3
"""Copy immutable Presence/World snapshots into a fenced MySQL candidate."""
...
from plugins.life_engine.storage.migration.domain_export import (
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--owner-id", default="presence-world-migrator")
...
async def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        run = await registry.create_run(
            ...
            metadata={
                "domain": "presence_world",
                "selection_contract": "explicit-operational-state-v1",
                "timestamps": "UTC-normalized",
                "json": "canonical-semantic",
        ...
        copy_report = await copy_presence_world_from_snapshot(
            args.snapshot,
            runtime,
        ...
        if args.reverse_export is not None:
            reverse_report = await export_presence_world_to_sqlite(
                runtime,
                template_snapshot_directory=args.snapshot,
                destination_directory=args.reverse_export,
            )
        health = {
            "presence": await stores.presence.health_snapshot(),
            "world": await stores.world.health_snapshot(),
        }
        verification = {
