...
- 往世乐土三号（任务 ID `019fc2ea-d63b-7b50-8f4a-532b22d777cc`）长期负责意识运行时、Presence、World State、共享主体上下文投影，以及学习系统的健康、架构与跨意识实例学习连续性；对应工程范围包括 `plugins/life_engine/learning/`、相关通用服务接线、测试和架构/运行文档。
- 学习系统负责人的职责是保证证据、候选洞察、Skill、知识投影、主体决策与运行健康可追溯，并确保潜意识只组织材料、不绕过意识实例替主体形成或改写第一人称自我；这项职责不授予其直接创作 `SOUL.md`、`USER.md`、`MEMORY.md` 或其他主体语义的权力。
- 学习系统需要 Memory 权威写入、通用存储 runtime、Voice、前端或其他领域接口时，仍按文件 owner 与“请求契约 → owner 实现 → 消费者集成 → 集成验收”流程协作，不得因长期职责而跨边界覆盖其他负责人的文件。
