...
前面十一章分别剖析了 Neo-MoFox-Soul 的课程定位、工程架构、主意识同步与各子系统实现。本章通过四个具体案例，**追踪状态序列 → 子系统响应 → 涌现行为的完整链条**，验证"系统涌现智能"原则（第 3 章 §3.3）在运行时的兑现程度。这四个案例并非完全实测数据，而是基于系统设计推演的**思想实验 + 局部代码追踪**；凡涉及数值预测之处均明确标注为"理论预期"或"示意值"。

---

## 12.1 案例 A：用户离开后的状态分化 — 5 分钟、30 分钟、3 小时
...
Neo-MoFox-Soul 的连续性原则（第 3 章 §3.1）要求：**即使在两次 LLM 调用之间，系统仍有正在变化的内在状态**（不变式 C2）。本案例追踪三个时长场景下 SNN 驱动、调质浓度的自然演化，并观察 LLM 在**无任何 if-else 判定时长逻辑**的条件下，如何产生情境适配的表达差异。
...
### 12.1.3 场景 A1：离开 5 分钟（300 秒）

**SNN 状态演化**（`snn/core.py:63 decay_only`）：

SNN 隐藏层膜电位遵循 LIF 泄漏方程（§6.2.1）：
...
1. **NREM 阶段**（`dream/scheduler.py:137`）：突触稳态缩减（SHY 算法，§9.3），记忆边弱化。
...
## 12.2 案例 B：习惯的形成 — 连续 12 天写日记

### 12.2.1 习惯追踪机制

Neo-MoFox-Soul 的习惯系统（`neuromod/engine.py:340 HabitTracker`）维护行为 streak（连续触发天数）与 strength（习惯强度）。强度计算公式（§7.6）：
...
## 12.3 案例 C：技术话题偏好的涌现 — 30 天 STDP 权重演化
...
## 12.4 案例 D：做梦后的灵感 — REM 激活扩散的跨概念连接

### 12.4.1 梦境生成流程

Neo-MoFox-Soul 的梦境系统（第 9 章）在睡眠窗口（如 02:00-07:00）或长时间空转时触发。核心流程：
...
本章通过四个案例——**用户离开时长差异**（案例 A）、**习惯形成轨迹**（案例 B）、**技术偏好学习**（案例 C）、**做梦后灵感**（案例 D）——展示了 Neo-MoFox-Soul 系统在多时间尺度、多子系统协作下的涌现行为。这些案例共同验证：
...
下一章将回到工程现实，压缩说明当前原型的主要局限、风险和后续工作。
