...
class TestLLMAPI:
    ...
    def test_create_embedding_request(self) -> None:
        """测试创建嵌入请求。"""
        with patch(
            "src.app.plugin_system.api.llm_api.EmbeddingRequest"
        ) as mock_request:
            mock_instance = MagicMock()
            mock_request.return_value = mock_instance
            mock_model_set = MagicMock(spec=ModelSet)
            ...
            result = llm_api.create_embedding_request(
                mock_model_set, "test", inputs=["text1", "text2"]
            )
    ...
    def test_get_model_set_by_task(self) -> None:
        """测试通过任务名获取 ModelSet。"""
        with patch(
            "src.app.plugin_system.api.llm_api.get_models_config"
        ) as mock_config:
            mock_model_set = MagicMock(spec=ModelSet)
            mock_instance = MagicMock()
    ...
    def test_get_model_set_by_name(self) -> None:
        """测试通过模型名获取 ModelSet。"""
        model_entry = {"model_identifier": "gpt-4"}
        with patch("src.kernel.config.models_loader.get_models_config") as mock_config:
            mock_instance = MagicMock()
            mock_instance.get_model_entry.return_value = model_entry
            mock_config.return_value = mock_instance
            ...
            assert result == [model_entry]
            mock_instance.get_model_entry.assert_called_once_with(
                "gpt-4", temperature=0.7, max_tokens=2000
            )

    def test_get_model_set_by_name_rejects_unregistered_model(self) -> None:
        """Direct model lookup must not fall back to the legacy registry."""

        with patch("src.kernel.config.models_loader.get_models_config") as mock_config:
            mock_instance = MagicMock()
            mock_instance.get_model_entry.return_value = None
            mock_config.return_value = mock_instance

            with pytest.raises(KeyError, match="未在 config/models.toml 中注册"):
                llm_api.get_model_set_by_name("missing")
