...
def test_body_limit_accepts_bodless_action_and_rejects_non_json_body() -> None:
    from fastapi import APIRouter, FastAPI
    from fastapi.testclient import TestClient

    from src.app.api.v1.runtime import BodyLimitMiddleware

    app = FastAPI()
    app.add_middleware(BodyLimitMiddleware)
    router = APIRouter()

    @router.post("/resources/{resource_id}:rotate")
    async def rotate(resource_id: str) -> dict[str, str]:
        return {"resource_id": resource_id}

    @router.post("/resources/{resource_id}:update")
    async def update(resource_id: str) -> dict[str, str]:
        return {"resource_id": resource_id}

    app.include_router(router)
    client = TestClient(app)

    # Body-less action endpoint must pass through without a 415.
    assert client.post("/resources/abc:rotate").status_code == 200
    # A request with a body must still require application/json.
    assert (
        client.post(
            "/resources/abc:update",
            content=b"raw-text",
