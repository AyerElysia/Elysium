...
def test_browser_uses_worklet_clocked_pcm_and_immediate_barge_in() -> None:
    html = (ROOT / "plugins/voice_live/static/voice_live.html").read_text(encoding="utf-8")
    ...
    assert "VL1" not in html  # The binary magic is emitted as exact bytes, not text guessing.
