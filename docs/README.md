...
- [安全部署脚本](./operations/deployment_scripts.md) — locked bootstrap、只读 doctor、手工前台 run 与不可覆盖备份
...
- [主体长期记忆治理与索引](./architecture/主体长期记忆治理与索引.md) — 有界 MEMORY、不可变长记忆边界、精确索引、主体决定与回忆留痕
...
- [主体主动性与外联](./architecture/主体主动性与外联.md) — 主体持续线索、一次性重新相遇、对象/表面分离、显式外联与跨实例连续性
- [主体上下文连续性与自述检查点](./architecture/主体上下文连续性与自述检查点.md) — 由 active consciousness 亲自决定释放边界并书写连续性，系统只负责容量事实、精确归档和机械硬预算
- [旧自主意向执行协议](./architecture/生命引擎自主意向执行协议.md) — 已退役 AutonomyIntent 的只读证据与兼容边界
