...
active generation fencing 只证明写者属于同一存储世代，不等于 singleton writer 所有权。selected Learning 与 `life_engine.runtime_context/global` 必须消费 `LifeEngineService` 从同一 `StorageBackendRuntime` 获得的 generation-scoped singleton writer claim；每次受保护写入都要在同一事务中复核 exact owner、lease epoch、fencing token 和数据库时间租约。失租立即 fail-closed，旧写者不得通过读取最新 projection/runtime state 后继续写。claim 的获得、续租与释放由 service 统一拥有，Learning 不自行打开 runtime 或另建租约。

Learning 的 claim scope 固定为 `life_engine.learning/selected_persistence`。业务启动先取得 runtime context claim，再取得 Learning claim，验证正式 MySQL guard triggers 已安装，之后才加载 runtime state 和初始化 scheduler；缺少 trigger 时启动 fail closed，不降级为仅应用层租约。两份 claim 使用同一 service boot writer identity，并由 runtime 统一续租和关闭释放。`learning_events` 的 INSERT 与 `learning_projections` 的 INSERT/UPDATE/DELETE 受 MySQL trigger guard：adapter 必须在 `unit_of_work(writer_claim=claim)` 内短暂绑定 exact claim，受保护语句结束后在 `finally` 清除 connection binding。只要该 scope 曾登记 claim，旧 adapter、raw SQL、过期 token 或错误 owner/epoch 即使仍属于 active generation 也不能继续写。local 使用同一事务前 claim 校验合同，不另建领域租约表。
