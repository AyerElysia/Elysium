...
服务 health 暴露 content-free 的 profile revision、逐资产 SHA-256、运行参数、算法延迟下限，以及有界滚动窗口中的平均/EWMA/P95/最大块耗时、模型 RTF 和实时余量。P95 达到块时长时标记 `overloaded`，达到 85% 时标记 `degraded`；这些状态只描述工程容量，不判断音色或表达质量。
