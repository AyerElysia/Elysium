...
> 文档状态：权威文档；意识 Presence、World Projection、Perception Gateway 与主体主动性权威已落地。
> 代码位置：`plugins/life_engine/core/`、`plugins/life_engine/service/` 与 `plugins/life_engine/initiative/`。
...
| 潜意识近期上下文 | committed LifeEngine history | 默认最近 5 个完整因果组；包含心跳想法、工具调用/结果和后台结果，不含私有聊天滚动原文 |
| 世界状态 | World Projection | 带来源 assertion、Presence 存在感和逐实例未确认 change；只进入当前 heartbeat turn |
| 待处理事件 | pending_events | 新消息、系统事件 |
| 认知机会 | CuriosityEngine | 有来源外部候选，不代表主体兴趣 |
| 最近聊天记录 | ChatStream | 近期对话上下文 |
| 最近文件修改 | Trace | workspace 变更 |
| 可达对象/表面 | Initiative Reachability | 稳定技术顺序、无最近流或重要性排序 |
| life 事件流 | LifeEventBus | 新增显著事件 |
| 学习进展 | LearningScheduler | 技能目录+自我认知 |
| 主体持续关注 | AttentionThread | 主体显式保持的公开线索 |
| 主体主动线索 | InitiativeSeed | 到期的一次性重新相遇，不是待执行计划 |
...
LifeChatter 是同一主体的**对外运行模式**——真实平台消息，或主体在当下显式选择对象与物理表面后形成的 outreach wake，会让对应表面的 LifeChatter 读取真实上下文。被唤醒不等于必须发送：表达或沉默仍由该意识实例当轮决定。它与心跳共享身份和记忆，但有独立的 LLM 上下文链（长生命周期 payload）。
...
| `LifeThinkAction` | 历史兼容组件，不再由 chat manifest 暴露 |
...
4. 打开唯一 ProactiveAuthority；旧 ThoughtStream 只保留离线只读归档
5. 三环自学习系统（LearningScheduler）
6. 启动统一权威的一次性重遇与 pending outreach 技术投递循环
