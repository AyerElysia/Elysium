...
│  主体主动线索  │  工具+Action     │  - 冲动候选 (ImpulseEngine)        │
...
4. 旧 ThoughtStream/Impulse 只读兼容与新 AttentionThread 接线
5. 三环自学习系统（LearningScheduler）
6. 绑定主体主动性 authority 并启动一次性重新相遇投递循环
...
| `initiative/authority.py` | — | 不可变决定账本、revision/CAS 与幂等回执 |
| `initiative/reachability.py` | — | 对象/物理表面分离与显式跨平台身份解析 |
| `initiative/tools.py` | — | 各意识实例共享的主体主动性工具 |
