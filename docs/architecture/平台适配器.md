...
> 代码位置：`plugins/napcat_adapter/`、`plugins/feishu_adapter/`。
...
平台适配器是数字生命与外部世界沟通的**感官通道**：将各平台的消息协议统一为内部事件流，并将内部决策翻译为平台原生消息发出。当前支持 QQ（NapCat/OneBot 11）和飞书。旧 N.E.K.O 展示面插件与 `/api/v1/surfaces` 已退役删除，不再作为通道。
...
QQ 有两条不同语义的语音能力，不能因为都能“在群里发声音”而混为一谈：

- `life_send_voice` 是主体的普通表达动作。它使用爱莉自己的本地 TTS 或已有音频，经核心 `voice` 段映射成 OneBot `record`，可以发送到当前 QQ 私聊或群聊；只有真实 `send_private_msg` / `send_group_msg` 回执成功才算交付。
- `platform_action(send_group_ai_record)` 调用 QQ 平台内置的角色语音服务。`character` 是 QQ 返回的角色身份，不是爱莉本地音色，也不能靠显示名称猜测；该能力保留给主体明确选择平台角色的场景。

能力说明只陈述接口身份，不替主体规定“应该说话、必须发语音或必须使用哪种音色”。主体决定表达方式后，基础设施只负责把所选动作准确送到当前目标。AdapterManager 会在构造适配器后注入 live CoreSink；NapCat 命令处理器必须在每次命令发送时消费当前 sink，确保查询和发送的成功/失败回执都能完成调用方等待，不能因构造期占位值丢失。
...
普通文件不是原生视觉、音频或视频模态。适配器接收附件后先把平台资源物化到受控的内容寻址存储 `data/received_files/<platform>/`，统一消息只携带文件名、字节数、SHA-256、`storage_key` 和本地路径等引用；文件正文、Base64 和下载 URL 不进入普通提示词、健康信息或错误日志。QQ、KOOK 与飞书共用这一合同：相同内容可幂等复用，写入采用临时文件加原子替换，单个入站附件硬上限为 200 MiB；下载失败或超限时只保留 `materialized=false` 的元数据，不能伪装成已经取得正文。

Life Chatter 取得引用后可由主体主动调用 `nucleus_save_media`，把附件复制到 Life Engine workspace；受控接收目录之外的任意本地路径不会被这个保存工具当作入站附件。`action-life_send_file` 则发送一个可读的普通本地文件，文件类型不由扩展名白名单裁决，但目录、通配符、空文件以及平台大小上限仍会显式拒绝。当前通用动作上限为 100 MiB；飞书消息文件 API 按平台合同进一步限制为非空且不超过 30 MiB。QQ、KOOK 与飞书已经具备离线契约实现，真实客户端链路仍必须逐平台验收后才可标记为可用。

消息类型只用于协议解析、资源取得和发送能力选择，不构成“收到文件后应不应该保存、阅读、回复或转发”的认知规则。是否查看、持久化或发送文件仍由同一主体结合当前上下文决定；系统不得按扩展名、平台或消息类型自动制造主体意图。
