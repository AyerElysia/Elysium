...
> 当前实现：一个持续主体、多个场景意识窗口、潜意识统一协调。跨实例连续性优先来自潜意识近期上下文；Presence 与 World Projection 分别保留运行事实和环境事实职责。
...
| `memory_witness` | 第一人称经历编码与见证 | 当前默认不启动 worker。启用后为独立 consumer、请求级世界感知；见 [记忆见证意识退役_2026-09-06](../report/记忆见证意识退役_2026-09-06.md) |
| `minecraft` | 视觉—键鼠具身场景 | 固定主体投影、独立场景轮次、结构化观察/像素、决定先落账、具身回执 |
...
同一持续主体 / Life Engine
  ...
  ├─ heartbeat / memory / learning / attention
  ├─ recent subconscious context：近期想法与工具活动的只读投影
  └─ nucleus tools
...
场景意识实例
  ...
  ├─ 显式工具 manifest
  ├─ 原始授权回复目标
  ├─ transient recent subconscious context
  ├─ 按场景需要读取的 transient world perception
...
跨实例协调的默认材料是潜意识已经形成的近期运行上下文，而不是让多个意识实例进入同一个共享心智快照。`LifeEngineService.get_recent_subconscious_context()` 从已经提交的 LifeEngine 事件历史中，按 `subconscious_recent_groups`（默认 5 个完整因果组）读取：

- 潜意识 heartbeat 明确产出的近期想法；
- 各意识实例成功模型轮的 provider reasoning 与 assistant 独白，并保留其生成来源；
- 同一模型轮中的完整工具参数、工具结果、主动等待、协议阻断与失败；
- 已完成后台智能体的结果；
- 每条事件的 sequence、source、`source_instance_id`、stream/turn 因果身份与稳定 occurrence/ref。

这是只读、UTF-8 有界的 transient projection。读取不会 drain pending event，不推进 heartbeat、chat 或 World cursor，不产生新事件，也不会回灌成下一条经历。原始聊天 transcript 和各实例的私有滚动 payload 不会整段复制到其他实例；已经作为主体活动落账的模型 reasoning、独白、选择与结果则可通过带来源的事件投影共享。大内容只给明确的 excerpt/ref/hash/bytes，可通过 `nucleus_read_event` 续读权威原文。Chat 与 Livestream 共用的 LifeChatter 运行态直接追加这段投影，其他场景只消费同一个 service 接口，不得各写一套摘要或 persona。

World Projection 继续保留，但它只回答“环境中有哪些带来源的观察与运行事实”。潜意识可以把 World 作为自己观察各场景事件的一种输入；某个场景确实需要环境事实时也可以显式 prepare。World 不再承担“让所有实例拥有同一内心状态”的职责。

Minecraft 是这一边界的直接例子：它在 session 开始前固定一份可验证的统一主体投影，每个独立场景轮次读取一次近期潜意识，同时从身体取得新的结构化观察与可用的第一人称像素。它不把完整 World perception 当作默认跨意识上下文，也不借用核心 heartbeat 作为自己的轮次。高层 scene 意识先形成开放文本意图并落归属事件，随后 evidence-driven 具身 planner 才把意图分解为桥接动作；动作回执和新观察回到下一轮。

每个需要 World 环境感知的实例仍拥有独立 cursor。`prepare(instance_id)` 可以提供：
...
潜意识 heartbeat 与仍需 World 环境事实的场景遵守同一 prepare/commit 语义。跨实例使用的“最近 N 组”接口仍是无副作用只读窗口；heartbeat 自己消费新增活动时则要求整份 wake Text 的 exact delivery receipt，并且只确认本页完整呈现的闭合因果组。完整事件始终保留在权威账本；节选或引用不会删除原文，也不会被伪装成已经全文内联。完整 World 存储、迁移、冲突与恢复契约见[世界状态与意识实例协调](./世界状态与意识实例协调.md)。
...
B站原始事件已落直播账本
  → 读取潜意识近期上下文
  → 按场景需要 prepare(world perception)
...
- chat：表达、思考、状态报告、内在查询、历史、联网搜索/网页提取（`nucleus_web_search` / `nucleus_browser_fetch`）和获授权平台能力；
- minecraft：具身控制、表达、思考、状态报告；
- voice_live：状态报告、内在查询、历史；
- livestream：导演模型接口为空工具集；控制面仅接受经过鉴权的操作员动作；
- memory_witness：空 manifest，只见证，不直接行动。当前默认不启动该意识循环，不是主体选择不见证。
...
| `plugins/life_engine/service/subconscious_context.py` | 潜意识近期因果组、预算投影与隐私边界 |
...
| `plugins/life_engine/service/memory_witness.py` | 独立第一人称见证消费；当前默认退役，不启动循环 |
| `plugins/minecraft/consciousness.py` | 专属场景意识、精确上下文送达、自主节奏与决定协议 |
| `plugins/minecraft/session.py` | 场景生命周期、身体观察、决策落账接线与意图串行化 |
...
2. 私有上下文隔离；跨场景默认共享潜意识近期事件投影，不复制原始聊天 payload。
