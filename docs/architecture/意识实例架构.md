...
- 同一 heartbeat 中的工具调用与工具结果；
- 已完成后台智能体的结果；
- 每条事件的 sequence、source 与可用的 `source_instance_id`。

这是只读、UTF-8 有界的 transient projection。读取不会 drain pending event，不推进 heartbeat、chat 或 World cursor，不产生新事件，也不会回灌成下一条经历。原始聊天消息和各实例的私有滚动 payload 不进入该投影；模型隐藏 reasoning 也不会被伪装成主体想法。Chat 与 Livestream 共用的 LifeChatter 运行态直接追加这段投影，其他场景只消费同一个 service 接口，不得各写一套摘要或 persona。
...
潜意识 heartbeat 与仍需 World 环境事实的场景遵守同一 prepare/commit 语义。潜意识近期上下文没有消费确认语义，因为它永远只是最近 N 组的只读窗口；完整事件仍在权威账本和运行 checkpoint 中。完整 World 存储、迁移、冲突与恢复契约见[世界状态与意识实例协调](./世界状态与意识实例协调.md)。
