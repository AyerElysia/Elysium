...
直播不是另一套人格，也不是把弹幕送给一个独立聊天机器人。它是爱莉希雅在直播场景中的一个 `ConsciousnessInstance`：使用同一个 `LifeChatter`、同一份灵魂与用户关系、同一个 `expression` 模型任务（导演配置默认 `actor` 别名）、同一条 Life Event / 记忆链。仓库不再保留独立的 `tasks.live` 路由。
...
- 导演送给模型的完整动态后缀必须作为单独 `Text` 注册 exact delivery；缺失、重复、裁剪或散列/字节数错配都不得生成可提交检查点。
...
每次导演模型成功生成后、在解析计划或产生舞台副作用之前，完整 provider reasoning 与
assistant message 会以当前 livestream consciousness instance、session、stream、输入事件 sequence
窗口和 transport request 身份写入统一意识活动账本。`director.decision` 继续表达可执行的场景决定，
统一活动事件则证明模型当轮实际生成过什么；两者不能相互替代。空文本、协议失败和中断也不会
被基础设施补写成主体表达。跨实例潜意识只消费有来源的有界投影，大正文通过 occurrence ref 续读。
...
生产实现把这笔确认固化为 `ChatterRuntimeCheckpoint`。导演只持久化最终完整动态后缀的 delivery ID、SHA-256、UTF-8 字节数、Life Event/思考前沿，以及一个不含正文的 `PerceptionCommitCheckpoint`；模型提示词、World 投影正文和观众文本均不会进入检查点。最终成功 attempt 的 `EffectiveContextReceipt` 必须证明该后缀作为唯一完整 `Text` 存在，随后才会把同样不含正文的 exact receipt 写入 `director.decision`。

决定落账后统一调用 `commit_chatter_runtime_delivery()`：它先验证最终后缀回执，再幂等补交 World 游标，最后持久化 Life Event/思考游标并清理匹配的 pending。即使进程在 World 已确认、运行态尚未落盘之间崩溃，重启后也只重放已保存的 content-free checkpoint；不会重新 `prepare`、不会保存提示词正文，也不会用“模型返回过文本”冒充完整上下文已送达。旧版只有位置窗口、没有 delivery 指纹和 exact receipt 的 `world_perception` 记录仅保持可读，不能推进任何游标。
