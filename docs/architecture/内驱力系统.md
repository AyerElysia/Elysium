...
旧 ImpulseEngine 仍保留为兼容模块，但不再作为默认 heartbeat 的注意入口。默认装配只通过一个
`AttentionOpportunitySection` 呈现 ThoughtStream 与 Curiosity 的现有线索；它的输出类型明确为
`attention_opportunity`，只提供候选证据，不产生 open/create、note/advance、pause/dormant 或
close/completed 决定，也不推荐具体工具。

active/open 只表示“存在一条未闭合线索”，不表示本轮发生了主体动作。因此：

- 仅存在 active/open 线索时，heartbeat idle 继续单调增加；
- 只读 `nucleus_manage_thought_stream(action=list)` 与主动休息不重置 idle；
- 只有本轮由主体实际选择的非被动工具动作才重置 idle；
- ThoughtStream、Curiosity 与旧 Leisure 不再各自重复注入同一推动。
...
`thought_deepen` 与 `curiosity_engage` 已从默认规则集退役，避免仅凭线索存在每 20/45 分钟重复
推动。未来 canonical AttentionThread 接入后仍沿用同一原则：producer 只提交机会，状态变更必须
来自活跃意识实例的显式决定。
