...
13. 本地快照、归档与恢复曾使用不同 Core SQLite 默认文件名；精确文件遍历还会解引用子目录符号链接。运行默认必须统一，非默认路径只能显式配置，不能靠文件存在性猜测或自动改名。
...
- 本地备份从已校验的 `database.sqlite_path` 派生 Core 库，保留非默认路径显式参数；快照层逐项拒绝符号链接、非普通文件和越界路径。新部署、统一归档扫描与隔离恢复的 Core 默认目标统一为 `Elysium.db`。
...
任务执行记录中的两批风险范围 pytest 分别为 `98 passed` 与 `62 passed`。拉取并整合远端 `bf6d50c2`、补齐 NapCat reverse WebSocket 鉴权合同并将原两个大提交拆为六个依赖有序提交后，最终完整离线回归命令 `uv run --frozen --no-sync --group dev python -m pytest test -q --no-cov -n 0` 得到 `4682 passed, 21 skipped, 2 warnings`；两条 warning 均来自第三方 `websockets` 弃用提示。`uv lock --check`、ShellCheck、Bash 语法、`compileall`、全范围确定性 Ruff `E9/F63/F7/F82` 与 `git diff --check` 均通过。当前 Linux 环境没有 `pwsh`，因此 `deploy.ps1` 由共享 Python 合同测试与静态审查覆盖，尚未在真实 PowerShell 解释器执行。
