...
- 当前已挂载 P3-01 的五个认证端点、P3-02 的 `/bootstrap`、`/capabilities`、`/readiness`、`/health`，以及 P3-04 的命令创建、列表、单项查询和受限取消端点；除 `/health` 仅用于 API 存活探测外，其余基础接口要求短时 Bearer 会话和对应 scope。事件与领域 handler 仍由后续阶段完成，不能因为命令内核或 OpenAPI 可访问就宣称全部领域动作可用；
