...
logger = logging.getLogger("minecraft.launcher")
...
class MCConfig:
    ...
    agent_bridge_uri: str = "ws://127.0.0.1:18768/elysium"
    agent_bridge_listen_uri: str | None = "ws://127.0.0.1:18768/elysium"
    agent_token_file: Path = field(
        default_factory=lambda: Path(
            "/mnt/g/Game/Minecraft/ElysiaClient/config/elysium_bridge.json"
        )
    )
    ...
    agent_game_directory: str = r"G:\Game\Minecraft\ElysiaClient"
    agent_launch_bat: str = r"G:\Game\Minecraft\ElysiaClient\LaunchElysia.bat"
    agent_expected_bridge_version: str = "0.3.0"
    agent_bridge_mod_filename: str = "elysium_bridge-0.3.0.jar"
    agent_expected_bridge_sha256: str = (
        "02765CCD262ADAE3DA2D6EE9AC2323E5B151F20773FF3687D3C73D5DD108AEFE"
    )
    ...
    def __post_init__(self) -> None:
        ...
        native_profile.offline_uuid(self.agent_shared_username)
        native_home = native_profile.mount_path(self.agent_game_directory)
        native_script = native_profile.mount_path(self.agent_launch_bat)
        if native_home == self.mc_home or self.mc_home in native_home.parents or native_home in self.mc_home.parents:
            raise ValueError("native client directory must be separate from the human installation")
        if native_script.parent != native_home:
            raise ValueError("native launch script must be inside its isolated game directory")
        if self.agent_shared_username == self.offline_username:
...
class MinecraftLauncher:
    ...
    async def check_installation(self) -> dict[str, Any]:
        """检查 MC 安装状态。"""
        shared = self._cfg.shared_world_enabled
        mc_home = (
            native_profile.mount_path(self._cfg.agent_game_directory)
            if shared else self._cfg.mc_home
        )
        launch_script = (
            native_profile.mount_path(self._cfg.agent_launch_bat)
            if shared else self._launch_script_path()
        )
        result: dict[str, Any] = {
            "mc_home": str(mc_home),
            ...
            "launch_bat": self._cfg.agent_launch_bat if shared else self._cfg.launch_bat,
        ...
        # 检查版本
        version_dir = self._cfg.mc_home / "versions" / "neoforge-21.1.219"
                ...
                if shared:
                    try:
                        native_profile.validate_launch_script(
                            content, game_directory=self._cfg.agent_game_directory,
                            username=self._cfg.agent_shared_username,
                            address=self.native_server_address(),
                        )
                    except ValueError as exception:
                        result["isolated_profile_ready"] = False
                        result["isolated_profile_error"] = str(exception)
                    else:
                        result["isolated_profile_ready"] = True
                        result["quick_play_configured"] = True
        ...
        if (
            ...
            and bridge_candidates[0].name == (
                self._cfg.agent_bridge_mod_filename if shared else self._cfg.bridge_mod_filename
            )
        ):
            ...
            result["bridge_mod_ready"] = (
                bridge_hash.casefold() == (
                    self._cfg.agent_expected_bridge_sha256 if shared
                    else self._cfg.expected_bridge_sha256
                ).casefold()
            )
        if (
    ...
    async def launch(self) -> LaunchResult:
        ...
        if self._cfg.shared_world_enabled:
            installation = await self.check_installation()
            if not all(installation.get(field) for field in (
                "isolated_profile_ready", "bridge_mod_ready", "baritone_mod_ready",
            )):
                return LaunchResult(success=False, error="isolated native client is not prepared")
            try:
                pid = await native_profile.find_process(
                    self._cfg.agent_game_directory, self._cfg.agent_shared_username,
                )
                reused = pid is not None
                if pid is None:
                    pid = await native_profile.launch_process(self._cfg.agent_launch_bat)
                self._running = True
                return LaunchResult(
                    success=True, pid=pid, reused_existing=reused,
                    window_title="Minecraft (isolated native account)",
                )
            except (OSError, RuntimeError, TimeoutError) as exception:
                return LaunchResult(success=False, error=str(exception))
    ...
    async def find_window(self) -> dict[str, Any] | None:
        """查找 MC 窗口。"""
        if self._cfg.shared_world_enabled:
            # Native identity and vision do not depend on desktop window discovery.
            return None
        return await self._bridge.find_window()

    def native_server_address(self) -> str:
        """Address as seen by the Windows client, not the WSL headless client."""

        host = "127.0.0.1" if self._cfg.bot_server_host == "auto" else self._cfg.bot_server_host
        return f"{host}:{self._cfg.bot_server_port}"
    ...
    def prepare_shared_world_bat(self) -> PureWindowsPath:
        """Reject the retired human-profile cloning API with an explicit migration."""
        raise RuntimeError(
            "prepare the isolated native profile explicitly; cloning the human launch script is disabled"
        )
