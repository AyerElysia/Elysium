...
class BridgeConfig:
    ...
    expected_player_name: str | None = None
    expected_player_uuid: str | None = None
    expected_game_directory: str | None = None
...
class MinecraftBridgeClient:
    ...
    def __init__(self, config: BridgeConfig) -> None:
        ...
        self._pending_frame: tuple[str, asyncio.Future[bytes]] | None = None
        self._retired_frame_ids: deque[str] = deque(maxlen=32)
            ...
            self._encode(
                {
                    ...
                    "last_observation_sequence": (
                        self._latest_observation.sequence
    ...
    async def capture_frame(self, *, max_width: int = 960) -> bytes:
        """Fetch one hash-verified PNG from this authenticated game's render target."""

        self._require_connected()
        if "vision.capture" not in self._capabilities:
            raise BridgeProtocolError("represented body has no native vision capability")
        if isinstance(max_width, bool) or not isinstance(max_width, int) or not 160 <= max_width <= 960:
            raise ValueError("native frame width must be between 160 and 960")
        async with self._frame_lock:
            request_id = f"frame_{uuid4()}"
            future: asyncio.Future[bytes] = asyncio.get_running_loop().create_future()
            self._pending_frame = (request_id, future)
            try:
                await self._send({
                    "type": "frame_request", "request_id": request_id,
                    "max_width": max_width,
                })
                async with asyncio.timeout(self._config.observation_timeout_seconds):
                    return await future
            finally:
                self._retired_frame_ids.append(request_id)
                self._pending_frame = None
                if not future.done():
                    future.cancel()

    def _accept_frame(self, message: Mapping[str, Any]) -> None:
        """Correlate a bounded sensory frame; late timed-out images cannot leak elsewhere."""

        request_id = str(message.get("request_id") or "")
        if message.get("instance_id") != self._instance_id:
            raise BridgeProtocolError("native frame came from another game instance")
        pending = self._pending_frame
        if pending is None or pending[0] != request_id:
            if request_id in self._retired_frame_ids:
                return
            raise BridgeProtocolError("native frame has no pending request")
        future = pending[1]
        if future.done():
            return
        if message.get("error"):
            future.set_exception(RuntimeError(f"native frame capture failed: {message['error']}"))
            return
        frame = message.get("frame")
        if not isinstance(frame, Mapping) or frame.get("mime_type") != "image/png":
            raise BridgeProtocolError("native frame must be a PNG object")
        if frame.get("source") != "minecraft-render-target":
            raise BridgeProtocolError("native frame source is not the represented render target")
        width, height = frame.get("width"), frame.get("height")
        if (
            isinstance(width, bool) or not isinstance(width, int) or not 1 <= width <= 960
            or isinstance(height, bool) or not isinstance(height, int) or not 1 <= height <= 4096
        ):
            raise BridgeProtocolError("native frame dimensions exceed their budget")
        encoded = frame.get("data_base64")
        if not isinstance(encoded, str) or len(encoded) > 6 * 1024 * 1024:
            raise BridgeProtocolError("native frame exceeds its encoded budget")
        try:
            data = base64.b64decode(encoded, validate=True)
        except (ValueError, binascii.Error) as exception:
            raise BridgeProtocolError("native frame is not valid base64") from exception
        if (
            len(data) > 4 * 1024 * 1024 or not data.startswith(b"\x89PNG\r\n\x1a\n")
            or hashlib.sha256(data).hexdigest() != frame.get("sha256")
        ):
            raise BridgeProtocolError("native frame content hash or PNG signature is invalid")
    ...
    def _validate_hello(self, hello: Mapping[str, Any]) -> None:
        ...
        for field, expected_value in (
            ("player_name", self._config.expected_player_name),
            ("player_uuid", self._config.expected_player_uuid),
        ):
            if expected_value is not None and hello.get(field) != expected_value:
                raise BridgeProtocolError(f"connected game has a different {field}")
        expected_directory = self._config.expected_game_directory
        if expected_directory is not None and PureWindowsPath(
            str(hello.get("game_directory") or "")
        ) != PureWindowsPath(expected_directory):
            raise BridgeProtocolError("connected game is not the isolated client directory")
    ...
    def _fail_pending(self, exception: BaseException) -> None:
        ...
        if self._pending_frame is not None and not self._pending_frame[1].done():
            self._pending_frame[1].set_exception(exception)
