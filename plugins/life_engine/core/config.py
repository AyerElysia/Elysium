...
class LifeEngineConfig(BaseConfig):
    ...
    class SettingsSection(SectionBase):
        ...
        message_checkpoint_async: bool = Field(
            default=True,
            description="消息落账后的 World 投影与 runtime checkpoint 是否后台执行；权威事件始终先耐久提交。",
        )
    ...
    class MemoryWitnessSection(SectionBase):
        ...
        enabled: bool = Field(
            default=False,
            description=(
                "是否启动记忆见证 worker（Experience 摄取、第一人称写作与投递）。"
                "当前默认关闭：操作者退役，不是主体选择不见证。"
                "关闭后不调用见证 LLM、不推进 author cursor；"
                "重新启用前必须先处理 oversized occurrence，禁止跳游标或截断。"
            ),
        )
        interval_seconds: int = Field(
            default=300,
    ...
    @config_section("opportunity")
    class OpportunitySection(SectionBase):
        """Explicit migration switch; subject capability choices live in storage."""

        enabled: bool = Field(
            default=False,
            description="统一机会运行时。需先完成独立schema与旧安排迁移；不自动安装能力。",
        )
        poll_interval_seconds: float = Field(
            default=5.0, ge=1.0, le=60.0,
            description="仅扫描工程到期事实的间隔，不是认知LLM执行频率。",
        )
