...
def _format_time(raw_time: float | int | None) -> str:
    ...
    if isinstance(raw_time, datetime):
        parsed = raw_time if raw_time.tzinfo else raw_time.replace(tzinfo=timezone.utc)
...
class EventBuilder:
    ...
    def build_tool_call_event(
        ...
        *,
        ...
        """构建工具调用事件。"""
        seq = self._next_sequence()
        event_id = f"tool_call_{uuid4().hex}"
    ...
    def build_tool_result_event(
        ...
        *,
        ...
        event_id = f"tool_result_{uuid4().hex}"
    ...
    def build_agent_result_event(
        ...
        *,
        ...
        seq = self._next_sequence()
        event_id = f"agent_result_{uuid4().hex}"
